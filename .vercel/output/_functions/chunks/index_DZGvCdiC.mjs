...
const GET = async ({ url }) => {
  ...
  let query = supabase.from("invoices").select("*, clients(company_name)", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
...
const POST = async ({ request }) => {
  try {
    ...
    if (error) return apiError(error.message, [], 500);
    return apiSuccess(data, "Invoice generated successfully", 201);
  } catch (err) {
