...
const GET = async ({ url }) => {
  ...
  let query = supabase.from("replacement_requests").select("*, clients(company_name), deployments(role, candidates(full_name))", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
